/*******************************************************************************
*   Version 1.031cilk December 2013                                            *
*                                                                              *
...
*******************************************************************************/
  ...
  flt8 x, minDeltaPhi, temp;
  int *lStart;
  flt8 *Qstart, *mapd, *norm, *calc1, *calc2, *calc3, *calc4;
  fftw_complex *mapc, *mapRow, *mapRowTrans;
  fftw_plan **plans;
  ...
  for( i = 0; i < coords.nThetaVals; i++)
  {
    ...
    fftw_complex *almPosPtr, *almNegPtr;
  ...
  if( !inputIsComplex )
  {
    cilk_for(int m = 1; m <= lmax; m++ )
    {
      fftw_complex *almPosPtr = alm + almIndex(m,m,lmax) - m;
      fftw_complex *almNegPtr = alm + almIndex(m,-m,lmax) - m;      
      
      if( m%2 )
      {
        cilk_for(int l = m; l <= lmax; l++ )
	{
...
*******************************************************************************/
  ...
  flt8 x, minDeltaPhi, sq4pin1;
  int *lStart;
  flt8 *mapd, *norm, *calc1, *calc2, *calc3, *calc4, *Qstart;
  fftw_complex *mapc, *mapRow, *mapRowTrans;
  fftw_plan **plans;
